...
def main():
    ...
    for index, (image_path, rotation) in enumerate(image_list):
        print(f"Extracting features for image {index + 1} of {len(image_list)}")

        (rgb_tensor, gray_tensor, size) = create_image_tensors(
        ...
        global_descriptors[image_path] = dir_output["global_descriptor"][0].cpu().numpy().astype(float32, copy=False)
        keypoints[image_path] = superpoint_output["keypoints"][0].cpu().numpy().astype(float32, copy=False)
        descriptors[image_path] = superpoint_output["descriptors"][0].cpu().numpy().astype(float32, copy=False)
