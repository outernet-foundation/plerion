    ...
    static public class PlerionAPI
    {
        ...
        public static async UniTask<List<LocalizationMapModel>> GetMapsWithinRadiusAsync(double latitude, double longitude, double height, double radius, Lighting lighting)
        {
            try
            {
                ...
                Log.Info(LogGroup.Localizer, $"Plerion API nearby maps response: {string.Join(", ", maps.Select(map => map.Name))}");

                return maps;
            }
            catch (Exception exception)
            {
                Log.Error(LogGroup.Localizer, "Plerion API nearby maps request failed", exception);
                return null;
            }
        }
        ...
        public static async UniTask<WifiGeolocationRecord[]> GetWifiGeolocation(params string[] bssids)
        {
            ...
            {
                Log.Error(LogGroup.Localizer, "Plerion API wifi geolocation request failed", exception);
                return null;
            }
        }
        ...
        public static async UniTask<float> GetHeight(double latitude, double longitude)
        {
            return 0;
        }
    }
