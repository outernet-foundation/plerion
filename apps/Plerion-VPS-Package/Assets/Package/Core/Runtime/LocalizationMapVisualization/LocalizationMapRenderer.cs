...
namespace Plerion.VPS
{
    ...
    public class LocalizationMapRenderer : MonoBehaviour
    {
        ...
        private async UniTask LoadMap(Guid id, CancellationToken cancellationToken = default)
        {
            ...
            var pointData = await VPSManager.API.GetMapPointsAsync(id, cancellationToken);
            ...
            await LoadPoints(ParsePointData(pointData).ToArray(), cancellationToken);
        }

        private IEnumerable<Vector3> ParsePointData(List<double> data)
        {
        ...
        private async UniTask LoadPoints(Vector3[] points, CancellationToken cancellationToken = default)
        {
