...
using UnityEngine;
using static Plerion.VPS.ReferenceFrame;
            ...
            IEnumerable<(double4x4 transform, float normalizedConfidence)> estimates)
        {
            ...
            foreach (var estimate in estimates)
            {
                var positionScore = math.exp(
                    ...
                    ReferenceFrame.RansacPositionInlierThreshold);

                var rotationScore = math.exp(
                    -math.abs(Quaternion.Angle(estimate.transform.Rotation(), rotation)) /
                    ReferenceFrame.RansacRotationInlierThreshold);

                score += Mathf.Pow(estimate.normalizedConfidence, ReferenceFrame.RansacConfidenceFactor) * (positionScore + rotationScore);
            }
