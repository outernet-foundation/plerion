...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.Hands
{
    ...
    public class ReleaseThresholdButtonReader : MonoBehaviour, IXRInputButtonReader
    {
        ...
        const float performedTimeThreshold = 0.05f;
        float performedTimer = -1;
        ...
        void Update()
        {
            ...
            {
                if (!m_ValueInput.ReadIsPerformed())
                {
                    performedTimer = -1;
                    newValue = false;
                }
                else
                {
                    if (performedTimer == -1)
                    {
                        performedTimer = Time.time;
                    }

                    newValue = Time.time - performedTimer >= performedTimeThreshold;
                }
            }
